...
#set_optimizer_attributes(m, "NonConvex" => 2,"Presolve"=>0) #ONLY for NLP
...
function create_supply(lambda, s, D_tot, sdPFAD, sdAF, sdUCO, avgPFAD, avgAF, avgUCO, sdMINPFAD, avgMINPFAD, sdMINAF, avgMINAF,sdMINWCO, avgMINWCO)
...
#a_b = [0.95 ;0.9; 0.99]; #oil percentages of biomass WCO, Animal fat, PFADs, used in bachelor thesis
...
PC_s, B_sb, D_s, x_sb_min= create_supply(lambda, s1, D_tot, sdPFAD, sdAF, sdUCO, avgPFAD, avgAF, avgUCO, sdMINPFAD, avgMINPFAD, sdMINAF, avgMINAF,sdMINWCO, avgMINWCO)


capillarity_factor=2; #give as input, test different levels

#CHANGE: splitting randomly not to even parts
function create_capillarity_expansion(capillarity_factor, PC_s, B_sb,D_s, s1, x_sb_min, r_low=50, r_high=100)
    ...
    copyMIN=x_sb_min[s1-UCO_suppliers+1:s1]
    newMINS=x_sb_min[1:s1-UCO_suppliers]
    ...
    new_s=1:new_s1
    return newB_sb, newPC_s, newD_s, new_s1, new_s, newMINS
end

B_sb, PC_s, D_s, s1, s, x_sb_min= create_capillarity_expansion(capillarity_factor, PC_s, B_sb,D_s, s1, x_sb_min)
...
#CB_b = [250, 400, 450]; #cost of biomass b, WCO , AF, PFAD , values used in bachelor thesis
CB_b = [90, 120, 100]; #values used in overleaf
#PC_b = [680 696 700]; #processing cost for biomass b, values used in bachelor thesis
